...
export default function EngineHighlight() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      // 1. GSAP SplitText Scroll Animation: Text sinh động, bay nhảy theo bước cuộn
      if (titleRef.current) {
        ...
        if (descRef.current) {
          splitDesc = new SplitText(descRef.current, {
            type: 'words',
            wordsClass: 'split-desc-word inline-block mr-1 will-change-transform',
          });
        }
        ...
        if (badgeRef.current) {
          textTl.fromTo(badgeRef.current, {
            scale: 0.8,
            opacity: 0,
            y: -25,
          }, {
        ...
        // Ký tự tiêu đề bay nhảy, xoay lật 3D sống động từ trung tâm tỏa ra
        textTl.fromTo(splitTitle.chars, {
          opacity: 0,
          y: (i) => 55 + (i % 4) * 14,
          rotateX: -85,
          rotateZ: (i) => (i % 2 === 0 ? -16 : 16),
          scale: 0.65,
        }, {
          opacity: 1,
          y: 0,
        ...
        // Lượn sóng nhấp nhô mượt mà khi cuộn tiếp
        textTl.to(splitTitle.chars, {
          y: (i) => Math.sin(i * 0.75) * 10,
          rotateZ: (i) => Math.cos(i * 0.75) * 4,
          stagger: {
            each: 0.02,
            from: 'start',
            yoyo: true,
  ...
  return (
    ...
    >
      ...
      <div className="relative max-w-7xl mx-auto z-10">
        ...
        {/* Minimalist, Clean, Luxury Headline (Synchronized with Upper Sections) */}
        <div ref={titleContainerRef} className="text-center max-w-3xl mx-auto mb-16 sm:mb-20 px-2 sm:px-4">
          <div 
            ref={badgeRef}
            className={`inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full text-[11px] font-bold tracking-[0.2em] uppercase mb-4 font-body border transition-colors ${
              isDark 
          ...
          <h2 
            ref={titleRef}
            className={`font-display text-3xl sm:text-5xl lg:text-6xl font-black tracking-tight uppercase leading-tight select-none ${
              isDark ? 'text-white' : 'text-slate-950'
            }`}
            style={{ perspective: '1000px' }}
          >
            ĐỘNG CƠ <span className="text-red-600 dark:text-red-500">eSP+ 330CC</span> THẾ HỆ MỚI
          </h2>

          <p 
            ref={descRef}
            className={`mt-3 text-xs sm:text-sm leading-relaxed max-w-2xl mx-auto font-body ${
